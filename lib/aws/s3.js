...
function S3 (options) {
  ...
  return {
    ...
    deleteObjects (Bucket, Keys, Quiet = false) {
    ...
    deleteObjectVersions (Bucket, Objects, Quiet = false) {
      debug(`Deleting ${Objects.length} Objects from: ${Bucket}`)
      return s3.deleteObjects({
        Bucket,
        Delete: {
          Objects
    ...
    getObjectStream (Bucket, Key, VersionId) {
      ...
      return s3.sdk.getObject(params)
    },
    ...
    listObjectVersions (Bucket, Key) {
      debug(`Listing Object Versions: ${Bucket} ${Key}`)
    ...
    // deletes all versions of an object
    removeObject (Bucket, Key, Quiet) {
      debug(`Deleting all versions of Object: ${Bucket} ${Key}`)
      return s3.listObjectVersions({
        Bucket,
        Key
      })
        .then((Versions) => s3.deleteObjects({
          Bucket,
          Delete: {
