...
ALB.prototype.describeLoadBalancer = function describeLoadBalancer(loadBalancerName) {
  ...
  return this.awsApi
    .then(api => Promise.all([
      ...
      this.describeTargetGroups({TargetGroupArns: [loadBalancer.targetGroupArn]})
        ...
        .then(LoadBalancerArn => LoadBalancerArn && api.describeLoadBalancerAttributes({LoadBalancerArn}))
    ]))
    .then(data => {
      if (data) {
        var result = {};
